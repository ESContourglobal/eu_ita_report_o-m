...
def get_df_plants(df: pd.DataFrame) -> pd.Series:
    if "Plant" not in df.columns:
        raise KeyError("Column 'Plant' not found in df")
    return (
        df["Plant"]
        .dropna()
        .astype(str)
...
def create_attachment_excel(report_path: str, month: int, year: int) -> Tuple[str, pd.DataFrame]:
    ...
    cols_to_drop = ["edis_total_prod_MWh", "best_total_prod_MWh"]
...
def build_and_send_email():
    ...
    recipients = ["francesca.rudello@contourglobal.com","angel.villamana@contourglobal.com"]
